...
def process_all(cfg: dict) -> pd.DataFrame:
    ...
    with session_scope(factory) as session:
        for pdf in sorted(pdf_dir.glob(cfg["entrada"]["padrao"])):
            ...
            method = (
                "ocr"
                if all(p["metodo"] == "ocr_pendente" for p in page_data)
            ...
            for page in page_data:
                ...
                if page["metodo"] == "ocr_pendente":
                    try:
                        ...
                        logging.exception(
                            "OCR falhou: %s p.%s", pdf.name, page["pagina"]
                        )
                        continue
                for raw in split_records(text):
                    fields = extract_fields(raw)
                    classification, reasons, normalized = validate_record(
                        fields, categories
                    )
                    protocol = (
                    ...
                    enriched = enrich_cep_record({**fields, **normalized}, cfg)
                    row = {
                        **fields,
                        "protocolo": protocol,
                        ...
                        "pagina": page["pagina"],
                        "metodo": page["metodo"],
                        "municipio": enriched.get("municipio"),
                        "uf": enriched.get("uf"),
                    }
                    rows.append(row)
